...
def _tab_spawner(config: cfg.Config, *, probe: bool = False) -> tuple[object | None, bool]:
    """Return ``(spawner, tabs_expected)`` for this host.

    The wt liveness probe (``wt.exe --version``) is never run: it opens a
    GUI help window (wt is a GUI app with no console), and the tiered
    launcher fallthrough in ``open_tab`` makes it redundant — a broken alias
    falls through to AUMID then console without popping any window. The
    windowless checks (wt_path + interop_registered) still catch truly
    headless contexts.
...
def _rescue_check(_args: argparse.Namespace) -> int:
    ...
    # tmux sessions even when the watchdog has not run yet. Gated by a
    # dedicated per-boot marker so the sweep runs once, not every shell.
    if tmux_spawner.available() and not rescue.already_revived(sd, boot_id):
        probe = process_probe.PsProcessProbe(config.get("interop_timeout_seconds"))
        store = JournalStore(sd)
        archive = ArchiveStore(sd)
        with mutation_lock(sd):
...
def _cmd_web(args: argparse.Namespace) -> int:
    ...
    handler = make_web_handler(
        ...
        zombie_strikes=config.get("zombie_strikes"),
        ...
        longpress_move_px=config.get("longpress_move_px"),
